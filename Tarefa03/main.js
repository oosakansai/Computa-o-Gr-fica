...
let txBola = 0.0; //posicao atual da bola
let tyBola = 0.0;
let txBola_offset = 0.005; // direcao, deslocamento da bola
...
function atualizaAnimacao(){
    ...
    // vou fazer a barra esquerda se mover primeiro para baixo e depois para cima, e a barra direita se mover primeiro para cima e depois para baixo.
    tyBE -= tyBE_offset;
    tyBD += tyBD_offset;

    if(tyBE > 0.8 || tyBE < -0.8)
        tyBE_offset = -tyBE_offset;

    if(tyBD > 0.8 || tyBD < -0.8)
        tyBD_offset = -tyBD_offset;
